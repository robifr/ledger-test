...
class EditCustomerFragment : CreateCustomerFragment() {
  ...
  override fun onViewCreated(view: View, savedInstance: Bundle?) {
    ...
    createCustomerViewModel.editResultState.observe(viewLifecycleOwner) {
      it.handleIfNotHandled(::_onResultState)
    }
  }

  private fun _onResultState(state: EditCustomerResultState) {
    state.editedCustomerId?.let {
      parentFragmentManager.setFragmentResult(
          Request.EDIT_CUSTOMER.key,
          Bundle().apply { putLong(Result.EDITED_CUSTOMER_ID_LONG.key, it) })
    }
    finish()
  }
