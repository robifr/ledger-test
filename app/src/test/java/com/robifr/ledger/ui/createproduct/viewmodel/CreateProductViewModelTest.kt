...
class CreateProductViewModelTest(
  ...
  private lateinit var _resultStateObserver: Observer<SafeEvent<CreateProductResultState>>
  ...
  fun beforeEach() {
    ...
    _viewModel = CreateProductViewModel(_productRepository, _dispatcher)
  ...
  fun `on save with created product`(createdProductId: Long) {
    ...
    if (createdProductId == 0L) {
      ...
      assertEquals(
          createdProductId,
          _viewModel.resultState.awaitValue().valueIfNotHandled?.createdProductId,
